...
module.exports = function (grunt) {
	...
	function initConfig() {
		function getValidateTasks(allTasks) {
			if (process.env.SETABLES_FAST === "true") {
				return [];
			}
			if (process.env.SETABLES_W3C_LOCAL_URL && allTasks) {
				return [
					"jshint",
					"karma:unit",
					// "htmlangular:index",
		...
		grunt.initConfig({
			...
			html2js: {
				options: {
					base: "<%= yeoman.dist %>",
					module: "SETABLES.html",
					quoteChar: "'"
				},
				main: {
					src: ["<%= yeoman.dist %>/**/*.html"],
					dest: "<%= yeoman.gen %>/SETABLESHtml.js"
				}
			},
			...
			htmlangular: {
				options: {
					...
					w3clocal: process.env.SETABLES_W3C_LOCAL_URL?process.env.SETABLES_W3C_LOCAL_URL:null,
					//i'm not sure why in local instalation these errors exists:
					relaxerror: process.env.SETABLES_W3C_LOCAL_URL?[
	...
	function getBrowsersForTest() {
		if (process.env.SETABLES_BROWSERS_FOR_TEST) {
			return process.env.SETABLES_BROWSERS_FOR_TEST.split(",");
		} else {
